 ...
 * @brief Actual LVGL v8 label/object rendering, static pages and UTF-8 text.
 * Layout: 28px title, seven 24px body rows, 28px footer. List uses five rows.
 */
...
#endif
uint8_t debug_ui_lvgl_pool[64U * 1024U];
...
/** @brief Prebuild every page and fixed text slot before reporting UI health. */
uint8_t debug_ui_view_init(DebugUiView *view)
{
    unsigned page_index, row_index;
    if (view == NULL) return 0U;
    memset(view, 0, sizeof(*view));
    view->group = lv_group_create();
    if (view->group == NULL) return 0U;
    lv_group_set_wrap(view->group, false);
    for (page_index = 0U; page_index < DEBUG_UI_PAGE_COUNT; ++page_index) {
        DebugUiViewPage *page = &view->pages[page_index];
        ...
        lv_obj_set_style_bg_color(page->root, lv_color_hex(0x101A27), 0);
        ...
        lv_obj_set_style_bg_color(page->title, lv_color_hex(0x203C51), 0);
        ...
        for (row_index = 0U; row_index < DEBUG_UI_VIEW_ROWS; ++row_index) {
            ...
            if (page->rows[row_index] == NULL) return 0U;
            lv_label_set_text_static(page->rows[row_index], page->row_text[row_index]);
        }
    ...
    lv_scr_load(view->pages[view->visible].root);
...
{
    text_set(page->rows[index], page->row_text[index], sizeof(page->row_text[index]), text);
}
/** @brief Draw an explicit text/colored focus marker, never the only state signal. */
static void highlight(DebugUiViewPage *page, unsigned index, uint8_t active)
{
    lv_color_t desired = lv_color_hex(active ? 0x234C61 : 0x101A27);
    if (lv_obj_get_style_bg_color(page->rows[index], 0).full != desired.full) {
        lv_obj_set_style_bg_color(page->rows[index], desired, 0);
        lv_obj_set_style_bg_opa(page->rows[index], LV_OPA_COVER, 0);
    }
}
                          ...
                          const DebugUiViewDiagnostics *diagnostics)
{
    ...
    case DEBUG_UI_PAGE_REVIEW:
        ...
        if (model->page == DEBUG_UI_PAGE_REVIEW) {
            ...
            footer = "左 取消  松开后长按确认";
        } else if (action->operation == DEBUG_UI_OPERATION_SET_MODE) {
            row(page, 6U, "中键查看切模确认");
            footer = "左 取消  中 查看确认";
        } else {
    ...
    for (index = 0U; index < DEBUG_UI_VIEW_ROWS; ++index) highlight(page, index, (uint8_t)((int)index == focus_row));
