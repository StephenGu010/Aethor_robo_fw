                                           ...
                                           size_t response_capacity)
{
    ...
    written_length = snprintf(
        response,
        response_capacity,
        "ok stage=%s key=%u accepted=%u mode=0x%02X ranges=0x%02X feedback=0x%02X enabled=0x%02X arrived=0x%02X motor_state=%u,%u start=%.4f,%.4f target=%.4f,%.4f pos=%.4f,%.4f vel=%.4f,%.4f fb_age=%lu,%lu fault=%u can_started=%u warning=%u passive=%u busoff=%u usb_drop=%lu probe_drop=%lu",
