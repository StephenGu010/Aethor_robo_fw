...
| 模式与连续反馈资格 | **正在进行**；原始模式 2，单次 DISABLE 后反馈很快过期；100 ms 脚本运行时 OUTPUT 关闭，不能计作带电验证 | 模式 1/恢复模式 2 均回读；失能和后续运行反馈的更新间隔、时延及分辨率支持选定控制周期 | 1–2 个工作日，取决于驱动反馈行为 |
