...
- 第一次有效按下：优先以两台当前位置 `+2π rad` 为目标；任一正向目标超过 PMAX 时，两台共同改为 `-2π rad`。
- 下一次有效按下：优先与上次方向相反；首选方向越界时改选共同安全方向，两个方向均不安全才锁存范围故障。
- 速度上限：`0.5 rad/s`。
- 到达后：继续发送终点位置保持。
- 重新受理条件：上一圈进入 `HOLDING` 且按键已经稳定释放；运动中按键不排队。
...
- 当前按键策略：启动检查完成前忽略按键；完成后每次完整按下/松开触发一圈，优先在 `+2π/-2π` 之间交替，并受两台 PMAX 共同约束。
...
6. 到达并稳定释放按键后再次短按，应看到 `moves` 加一、`next_dir` 翻转，并再次进入 `ENABLING → MOVING → HOLDING`。
...
| `READY/HOLDING` 但按键不受理 | 检查上一圈后是否稳定释放至少 20 ms，以及新按下是否形成稳定 20 ms 有效沿 |
| 按键后 `fault=3` | 两台电机在正反两个方向都没有共同的一圈安全空间；检查当前位置和 PMAX |
