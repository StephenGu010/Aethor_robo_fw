...
本工程以 STM32H723VGT6、FreeRTOS、USB CDC 和 FDCAN1 为基础。当前运行入口由 PA15 按键触发两台 S3519 各转一圈，方向按正向、反向依次交替；USB CDC 只承担查询和探针输出。原七轴关节控制模块仍保留，但当前不参与运行。

> 当前烧录目标为 PA15 双电机按键演示：第一次有效按下使电机 `0x1/0x2` 分别从当前位置增加 `2π rad`，到达并释放按键后，下一次有效按下分别减少 `2π rad`，之后正反交替。速度上限为 `0.5 rad/s`，每圈到达后保持。
...
然后短按并松开 KEY1。每次受理后串口阶段应依次进入 `ENABLING → MOVING → HOLDING`。第一次为正向一圈，第二次为反向一圈，之后交替。理论匀速一圈约 12.57 秒，驱动器自身加减速会使实际时间略长；30 秒未到达会锁存故障。
