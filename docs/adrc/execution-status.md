...
- 同一上电窗口复测时，电机 7 原始模式 2 发现完成，失能切入模式 1 并等到 `lcd_idle=1 can_idle=1 mit_ready=1`；`adrc acquire motor=7` 被接受，但约 100 ms 后返回 `owner=lcd handoff=timeout`。脚本未发送 `adrc release`，而是在 LCD 持权下恢复模式 2、最终 DISABLE。记录 `output/adrc/hardware/handoff_window_20260923/motor7_handoff_20260923T083939471Z.txt`；随后只读状态仍为 `enabled=00 moving=0 owner=lcd busoff=0`，见同目录 `posttimeout_readonly.txt`。用户已确认关闭 24 V。断电后只读 STM32 SRAM：本次挑战 `probe_submitted_us=400298000`、`probe_transmitted=1`、`probe_failed=0`，记录为同目录 `probe_ram_after_timeout.json`；说明禁能挑战已提交且专用 FDCAN 确认实际发送，但在 100 ms 内**没有得到合格的新鲜禁能反馈**。这次没有按挑战窗口单独记录原始 RX/接收解码，所以不能断言驱动完全未回复，更不能放开接管或 ADRC 运动。现已离线补上挑战窗口的原始帧、有效帧和拒收帧计数，并让单窗口脚本在超时后保存 `adrc gate`；主机测试通过，Keil 完整重建 0 错误、0 警告，日志为 `output/adrc/hardware/handoff_window_20260923/acquire_rx_diagnostic_build_20260923.log`。诊断版 `da23413` 已在 24 V 关闭时烧录：原 Flash 1 MiB 已备份，新镜像 320948 字节回读一致并复位运行，报告为 `output/adrc/hardware/handoff_window_20260923/20260923T165301/flash-result.json`。同目录 COM4 只读验收记录显示 `enabled=00 moving=0 owner=lcd busoff=0`，诊断计数均为零。尚未带电复测接管；本次烧录不批准 ADRC 运动。

- 用户再次确认 OUTPUT 开启、面板约 24 V / 0.10 A 后，诊断版在单次非运动窗口复测。模式 2 参数发现、失能切到模式 1 和 CAN 静默门均通过；`adrc acquire` 被接受，但约 100 ms 后仍为 `owner=lcd handoff=timeout`。新增计数为 `acq_submit=1 acq_tx=1 acq_failed=0 acq_rx=0 acq_valid=0 acq_rejected=0`：专用禁能挑战已被 FDCAN 实际发送，**该窗口内没有任何进入应用层的原始 CAN 帧**，因此不是解码拒收造成的接管失败。脚本在 LCD 持权下恢复模式 2、最终 DISABLE，未发送 ENABLE、转矩或速度指令；完整原始记录为 `output/adrc/hardware/handoff_window_20260923/motor7_handoff_20260923T085708664Z.txt`。用户已确认关闭 24 V。厂商 S3519 V1.1 手册第 7–9 页定义问询式状态反馈和 MIT 控制帧，第 8 页反馈状态码 0 为失能；但本机对 4/8 字节 `0xCC` 查询及模式 1 下的重复 DISABLE 都没有得到禁能反馈，不能用手册一般描述取代当前实测。当前不批准 ADRC 接管或运动。

- 基于上述证据，已离线实现两段接管挑战：先在专用 CAN 缓冲区发送 DISABLE；若没有因此取得合格反馈，只有收到实际发送成功回执才追加一帧 `Kp=0、Kd=0、速度目标=0、前馈转矩请求=0` 的 MIT 控制帧；必须取得最近一次挑战提交后的新鲜、失能、无故障、低速反馈及相应发送成功回执才转移控制权，否则超时保持 LCD。由于 12 位双极性映射不能精确表示零，按当前 `TMAX=10 Nm`，零转矩请求会截断为约 `-0.002442 Nm` 的名义编码值；因此它是**小幅控制帧，不是纯只读帧或物理零转矩保证**。硬件脚本新增显式批准开关；若收到使能、故障或运动反馈，优先再次 DISABLE，跳过模式恢复并要求人工关电。无反馈的主机超时、有效失能反馈、失败发送、控制权隔离等集成测试通过，Keil 全量构建 0 错误、0 警告，日志为 `output/adrc/hardware/handoff_window_20260923/neutral_acquire_build_20260923.log`。版本 `0a4e00b` 已在 24 V 关闭时烧录：原 Flash 1 MiB 已备份，新镜像 321136 字节回读一致，报告为 `output/adrc/hardware/handoff_window_20260923/20260923T171008/flash-result.json`；复位后同目录 COM4 只读记录显示 `enabled=00 moving=0 owner=lcd busoff=0`。尚未带电执行新的 MIT 控制帧。
